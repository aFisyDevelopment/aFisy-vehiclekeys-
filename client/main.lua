                ...
                if driver ~= 0 and not IsPedAPlayer(driver) and not HasKeys(plate) and not carIsImmune then
                    if IsEntityDead(driver) then
                        if not isTakingKeys then
                            ...
                            TriggerServerEvent('afisy-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 1)
                            QBCore.Functions.Progressbar("steal_keys", Lang:t("progress.takekeys"), 2500, false, false, {
                                ...
                                TriggerServerEvent('afisy-vehiclekeys:server:AcquireVehicleKeys', plate)
                                isTakingKeys = false
                            end, function()
                                isTakingKeys = false
                            end)
                    ...
                    elseif Config.LockNPCDrivingCars then
                        TriggerServerEvent('afisy-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 2)
                    else
                        TriggerServerEvent('afisy-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 1)
                        TriggerServerEvent('afisy-vehiclekeys:server:AcquireVehicleKeys', plate)
...
RegisterCommand('motor', function()
    TriggerEvent("afisy-vehiclekeys:client:ToggleEngine")
end)
...
RegisterNetEvent('afisy-vehiclekeys:client:AddKeys', function(plate)
    KeysList[plate] = true
...
RegisterNetEvent('afisy-vehiclekeys:client:RemoveKeys', function(plate)
    KeysList[plate] = nil
end)

RegisterNetEvent('afisy-vehiclekeys:client:ToggleEngine', function()
...
RegisterNetEvent('afisy-vehiclekeys:client:GiveKeys', function(id)
            ...
            if id and type(id) == "number" then -- Give keys to specific ID
                ...
                if IsPedSittingInVehicle(PlayerPedId(), targetVehicle) then -- Give keys to everyone in vehicle
                    ...
                    for p=1,#otherOccupants do
                        TriggerServerEvent('afisy-vehiclekeys:server:GiveVehicleKeys', GetPlayerServerId(NetworkGetPlayerIndexFromPed(otherOccupants[p])), targetPlate)
                    end
...
function GiveKeys(id, plate)
    ...
    if distance < 1.5 and distance > 0.0 then
        TriggerServerEvent('afisy-vehiclekeys:server:GiveVehicleKeys', id, plate)
    else
...
function GetKeys()
    QBCore.Functions.TriggerCallback('afisy-vehiclekeys:server:GetVehicleKeys', function(keysList)
        KeysList = keysList
    end)
end
    ...
    for job, v in pairs(Config.SharedKeys) do
	    ...
	    for _, vehicle in pairs(v.vehicles) do
		    ...
		    if not HasKeys(vehPlate) then
		        TriggerServerEvent("afisy-vehiclekeys:server:AcquireVehicleKeys", vehPlate)
		    end
		    return true
	        end
            end
        ...
        if not isBlacklistedVehicle(veh) then
            if HasKeys(QBCore.Functions.GetPlate(veh)) or AreKeysJobShared(veh) then
                ...
                if vehLockStatus == 1 then
                    TriggerServerEvent('afisy-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 2)
                    QBCore.Functions.Notify(Lang:t("notify.vlock"), "primary")
                else
                    TriggerServerEvent('afisy-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 1)
...
function LockpickFinishCallback(success)
    ...
    if success then
        ...
        if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
            TriggerServerEvent('afisy-vehiclekeys:server:AcquireVehicleKeys', QBCore.Functions.GetPlate(vehicle))
        else
            QBCore.Functions.Notify(Lang:t("notify.vlockpick"), 'success')
            TriggerServerEvent('afisy-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(vehicle), 1)
        end
    ...
    if usingAdvanced then
        if chance <= Config.RemoveLockpickAdvanced then
            TriggerServerEvent("afisy-vehiclekeys:server:breakLockpick", "advancedlockpick")
        end
    else
        if chance <= Config.RemoveLockpickNormal then
            TriggerServerEvent("afisy-vehiclekeys:server:breakLockpick", "lockpick")
        end
    end
end
