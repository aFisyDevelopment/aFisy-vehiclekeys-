...
RegisterNetEvent('afisy-vehiclekeys:server:GiveVehicleKeys', function(receiver, plate)
    local giver = source
...
RegisterNetEvent('afisy-vehiclekeys:server:AcquireVehicleKeys', function(plate)
    local src = source
    GiveKeys(src, plate)
end)

RegisterNetEvent('afisy-vehiclekeys:server:breakLockpick', function(itemName)
...
RegisterNetEvent('afisy-vehiclekeys:server:setVehLockState', function(vehNetId, state)
    SetVehicleDoorsLocked(NetworkGetEntityFromNetworkId(vehNetId), state)
end)

QBCore.Functions.CreateCallback('afisy-vehiclekeys:server:GetVehicleKeys', function(source, cb)
...
function GiveKeys(id, plate)
    ...
    TriggerClientEvent('afisy-vehiclekeys:client:AddKeys', id, plate)
end
...
function RemoveKeys(id, plate)
    ...
    TriggerClientEvent('afisy-vehiclekeys:client:RemoveKeys', id, plate)
end
...
QBCore.Commands.Add("geefsleutel", Lang:t("addcom.givekeys"), {{name = Lang:t("addcom.givekeys_id"), help = Lang:t("addcom.givekeys_id_help")}}, false, function(source, args)
	local src = source
    TriggerClientEvent('afisy-vehiclekeys:client:GiveKeys', src, tonumber(args[1]))
end)
